...
public:
	...
	bool wait_frame();
	bool begin_frame();
	bool end_frame(VkCommandBuffer* external_command_buffer, VkExtent2D input_extent, int input_image_index, bool waterwarp);
	...
	bool menu_button_down_[NUM_CONTROLLERS] = { false, false };

	bool XA_button_down_[NUM_CONTROLLERS] = { false, false };
	bool XA_touch_down_[NUM_CONTROLLERS] = { false, false };

	bool BY_button_down_[NUM_CONTROLLERS] = { false, false };
	bool BY_touch_down_[NUM_CONTROLLERS] = { false, false };
...
private:
	...
	bool render_composition_layer(std::vector<XrCompositionLayerProjectionView>& projection_layer_views, XrCompositionLayerProjection& composition_layer, VkCommandBuffer* external_command_buffer, VkExtent2D input_extent, int input_image_index, bool waterwarp);
	void render_projection_layer_view(const XrCompositionLayerProjectionView& projection_layer_view, const XrSwapchainImageBaseHeader* swapchain_image, int64_t swapchain_format, int view_id, int input_image_index, VkCommandBuffer* external_command_buffer, VkExtent2D input_extent, bool waterwarp);
